...
\end{align}
The reference length $R_\mathrm{ref} = 150$\,mm corresponds to half the gap between the pole pieces. For cylindrical pole-piece electromagnets, the field is strongest on the symmetry axis (where the pole faces are closest) and decreases toward the radial edges of the gap, corresponding to $c_2 > 0$. This is the generic configuration for all iron-core laboratory electromagnets operating below magnetic saturation~\cite{Rich1981}. For the Mills/Ritter electromagnets, the reported non-uniformity was $> 10$\,ppm~\cite{Ritter1984}.
...
We note that with $N = 5 \times 10^5$ sampling points, the spatial averages converge to their analytic values with negligible statistical fluctuation ($< 10^{-3}$\,MHz). The calculation is therefore effectively deterministic; the random sampling serves as a numerical integration method for the spatial average over the cavity volume. The near-exact linearity of $\delta\Delta_\mathrm{HFS}$ with $c_2$ visible in Table~\ref{tab:results} reflects the dominance of the first-order mechanism $\delta\nu \propto \langle \delta B \rangle \propto c_2$, not an artifact of insufficient sampling.
...
Table~\ref{tab:results} presents the baseline-corrected shift $\delta\Delta_\mathrm{HFS}$ for the Mills/Ritter configuration ($B_0 = 0.783$\,T, $\nu_\mathrm{MW} = 2338$\,MHz, $\gamma = 5$\,MHz, $R_\mathrm{cav} = 100$\,mm, $L_\mathrm{cav} = 200$\,mm). The cavity dimensions are estimated from the experimental descriptions in Refs.~\cite{Mills1975,Ritter1984}; the sensitivity to these values is assessed in Sec.~\ref{sec:sensitivity}. Table~\ref{tab:results} shows results for various field models and positronium distributions.
...
\textit{Cavity size dependence.}---The shift scales approximately as $R_\mathrm{cav}^{2.8}$. The deviation from the naive $R^2$ scaling expected from $\langle r^2 \rangle \propto R^2$ reflects the contribution of higher moments of the field distribution and the nonlinearity of the Breit-Rabi inversion. For the Mills/Ritter cavity ($R = 100$\,mm), $\delta\Delta_\mathrm{HFS} = -1.84$\,MHz at $c_2 = 15$\,ppm and $\lambda = 30$\,mm; for the Ishida cavity ($R = 64$\,mm) under the same field parameters, the shift is only $-0.47$\,MHz. Larger cavities sample more of the non-uniform field, amplifying the effect.
...
Ishida \textit{et~al.}~\cite{Ishida2014} identified incomplete thermalization of positronium as a systematic of approximately 10\,ppm ($\sim 2$\,MHz). Our field inhomogeneity bias is a \emph{distinct} effect: the non-thermalization changes the spatial distribution of Ps within the cavity, while the field inhomogeneity changes the field that each Ps atom experiences. However, the two effects are \emph{coupled}: non-thermalized Ps preferentially samples regions of larger field deviation, amplifying the inhomogeneity bias by a factor of 3--4 (Table~\ref{tab:results}). This coupling has not been previously considered.

The relationship between the two effects requires careful treatment. When we use a non-thermalized Ps distribution ($\lambda = 30$\,mm), our simulation already captures the coupling: the non-uniform spatial sampling amplifies the field-inhomogeneity bias. The Ishida thermalization correction of $\sim 2$\,MHz, estimated independently, therefore partially overlaps with our non-thermalized result. The two corrections should not be added directly. A conservative lower bound on the total systematic is given by the uniform-Ps result ($-0.61$\,MHz at $c_2 = 20$\,ppm), which excludes thermalization effects entirely. The upper bound is given by the non-thermalized result ($-2.40$\,MHz at $c_2 = 20$\,ppm), which includes some but not all of the thermalization-related shift. A self-consistent treatment requires simultaneous modeling of thermalization dynamics and field sampling, which we defer to future work.
...
We also neglect the non-Hermitian corrections from singlet annihilation, which Mills~\cite{Mills1983} showed produce a line-shape asymmetry and a line-center shift of $\sim 0.5$\,MHz. This correction affects both the uniform-field and non-uniform-field cases similarly, so it largely cancels in our baseline-subtracted result [Eq.~(\ref{eq:shift})]. A complete treatment would include these corrections self-consistently, but the effect on $\delta\Delta_\mathrm{HFS}$ is expected to be small ($< 0.1$\,MHz).
...
These results suggest that the long-standing positronium HFS anomaly could plausibly be a systematic artifact of the magnetic field environment rather than an indication of physics beyond the Standard Model. A definitive resolution requires either (i) a new measurement with a superconducting magnet and verified field uniformity at the $< 1$\,ppm level, or (ii) a magnetostatic reconstruction of the original electromagnets to determine the actual field profile.
